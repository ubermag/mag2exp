...
def test_sans_analytical_parallel_bloch():
    ...
    sans = mag2exp.sans.cross_section(m, method="unpol").sel(k_z=0)
...
def test_sans_analytical_perpendicular_neel():
    ...
    m = df.Field(mesh, nvdim=3, value=m_fun)
    sans = mag2exp.sans.cross_section(m, polarisation=[1, 0, 0], method="unpol").sel(
        k_z=0
    )
    idx = np.unravel_index(sans.array.argmax(), sans.array.shape)[0:2]
    q = sans.mesh.index2point(idx)[0]
    ...
    sans = mag2exp.sans.cross_section(m, polarisation=[1, 0, 0], method="pp").sel(k_z=0)
    assert np.isclose(sans.array, 0).all()

    sans = mag2exp.sans.cross_section(m, polarisation=[1, 0, 0], method="pn").sel(k_z=0)
    idx = np.unravel_index(sans.array.argmax(), sans.array.shape)[0:2]
    q = sans.mesh.index2point(idx)[0]
    assert np.isclose(abs(q), 1 / qx)
    peaks = (sans.array > 10).sum()
...
def test_sans_chiral_parallel():
    ...
    m = df.Field(mesh, nvdim=3, value=m_fun)
    cf = mag2exp.sans.chiral_function(m).sel(k_z=0)
...
def test_sans_chiral_perpendicular():
    ...
    m = df.Field(mesh, nvdim=3, value=m_fun)
    cf = mag2exp.sans.chiral_function(m, polarisation=[1, 0, 0]).sel(k_z=0)
